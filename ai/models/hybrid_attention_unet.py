...
class CoordinateInterSliceAttention(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int = 8, mode: SkipMode = "coord_only"):
        super().__init__()
        self.mode = mode
        if mode == "identity":
            return
...
def _make_skip_enhancer(channels: int, skip_mode: SkipMode) -> nn.Module:
...
class HybridAttentionUNet3D(nn.Module):
    ...
    def _apply_skip(self, skip_module: nn.Module, skip_feat: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        u4 = self.up4(b)
        d4 = self.dec4(torch.cat([u4, self._apply_skip(self.skip4, e4)], dim=1))
        u3 = self.up3(d4)
        d3 = self.dec3(torch.cat([u3, self._apply_skip(self.skip3, e3)], dim=1))
        u2 = self.up2(d3)
        d2 = self.dec2(torch.cat([u2, self._apply_skip(self.skip2, e2)], dim=1))
        u1 = self.up1(d2)
        d1 = self.dec1(torch.cat([u1, self._apply_skip(self.skip1, e1)], dim=1))
        return self.final(d1)


def build_model(config: ExperimentConfig | int | None = None) -> HybridAttentionUNet3D:
    if isinstance(config, ExperimentConfig):
        return HybridAttentionUNet3D(
            in_channels=1,
    ...
    return HybridAttentionUNet3D(in_channels=1, out_channels=num_classes)
